Текущий статус проекта: **v0.7.1 —  Реализован модуль Логистики и Завершения сделок**.
Готовы модули `Auth` (JWT, регистрация), `References` (кэширование в Redis), `Users` (профиль), `Companies` (проверка ИНН), `Team Management` (управление сотрудниками), `Catalog` (управление товарами и поиск), `Documents` (Генерация PDF счетов и договоров, загрузка в S3), `Finance` (Логика Эскроу, транзакции, расчет комиссий), `Trade` (Запросы RFQ, Коммерческие предложения, Сделки, Стейт-машина статусов, Триггеры защиты БД) и `Logistics` (Отслеживание доставки, интеграция со СДЭК, автоматическое закрытие сделок и выплата средств).
...
*   `npm run test:e2e`: Запуск сквозных (End-to-End) тестов.
*   `npx prisma migrate reset`: Полный сброс базы данных (удаление всех данных) и повторный накат миграций + сидинг.