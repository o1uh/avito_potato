Текущий статус проекта: **v0.5.0 —  Реализованы Документооборот и Финансовое ядро**.
Готовы модули `Auth` (JWT, регистрация), `References` (кэширование в Redis), `Users` (профиль), `Companies` (проверка ИНН), `Team Management` (управление сотрудниками), `Catalog` (управление товарами и поиск), `Documents` (Генерация PDF счетов и договоров, загрузка в S3) и `Finance` (Логика Эскроу, транзакции, расчет комиссий).
