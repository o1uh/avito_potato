Текущий статус проекта: **v1.4.0  — Реализован полный торговый цикл (Trade) и система уведомлений**.
...
- `Catalog`: Витрина товаров с фильтрацией (Elasticsearch), пагинацией и умной галереей (общие фото + фото вариантов).
- `Product Wizard`: Многошаговая форма создания товара с динамическими вариантами (SKU), автогенерацией артикулов и Drag-n-Drop загрузкой изображений.
- `Notifications` (NEW): Real-time уведомления (WebSockets), "колокольчик" в шапке с бейджем, всплывающие Toast-сообщения.
- `Trade Desk` (NEW): Рабочее место менеджера с Канбан-доской сделок и табами для RFQ (Запросов) и Офферов.
- `Deal Execution` (NEW): Детальная страница сделки с визуализацией этапов (Stepper), интерфейсы для оплаты, логистики (ввод трек-номера) и приемки товара.
...
- `Catalog`: Управление товарами (вариативность SKU), синхронизация с Elasticsearch, сложный поиск (Wildcard/Multi-match), загрузка изображений в MinIO.
