Текущий статус проекта: **v0.8.0 —  Реализован функционал Администрирования, Аналитики и Споров**.
Готовы модули:
...
- `Finance`: Эскроу-счета, транзакции, хранимые процедуры БД для атомарности, расчет комиссий.
- `Trade`: Полный цикл сделки (RFQ -> КП -> Сделка), стейт-машина статусов, логистика (интеграция СДЭК).
- `Governance` (NEW): Система Арбитража (споры по сделкам), Отзывы и Рейтинги компаний (DB Triggers).
- `Analytics` (NEW): Сбор статистики (Cron), Дашборды для пользователей и админа, Экспорт отчетов (CSV).
- `Admin` (NEW): Панель модерации товаров, блокировка пользователей/компаний.
